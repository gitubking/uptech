...
// Route temporaire pour réinitialiser un mot de passe / lister les utilisateurs
// Supprimer ce fichier après utilisation
export async function POST(request: NextRequest) {
  const { email, newPassword, secret, list } = await request.json()
  ...
  if (list) {
    return NextResponse.json({ users: users.map(u => ({ id: u.id, email: u.email, created_at: u.created_at })) })
  }
